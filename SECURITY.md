...
| CV rewrite output | Medium | Generated by backend providers, returned to client, not persisted by the repository backend. |
...
- Exposes `POST /rewrite-cv` with strict request and response validation for CV rewrite workflows.
...
- Validate CV rewrite output against `CvRewriteResult` before responding.
- Bound scores to expected ranges.
- For rewrite workflows, forbid invented experience, fabricated metrics, unsupported tools, unsupported seniority, and unsupported ownership claims.
- Use placeholders such as `[insert measurable result]` when truthful metrics are missing.
